...
public class FunctionManager : Singleton<FunctionManager>
{
    ...
    // obj의 자식들 Button ActiveSelft(false)
    public void InitChildButton(GameObject[] btnObj, bool isActive)
    {
    ...
    // Animation Curve 이용한 오브젝트 포물선 이동
    public IEnumerator ObjectParabola(GameObject start, GameObject target, AnimationCurve animationCurve, float speed)
    {
        Vector3 startPos = start.transform.position;
        Quaternion startRot = start.transform.rotation;

        Vector3 targetPos = target.transform.position;
        Quaternion targetRot = Quaternion.Euler(startRot.eulerAngles.x + 90, startRot.eulerAngles.y, startRot.eulerAngles.z);
        ...
        while (time < speed)
        {
            ...
            float linear = time / speed;
            float curveHeight = animationCurve.Evaluate(linear);
            float height = Mathf.Lerp(0, 1, curveHeight);

            start.transform.position = Vector3.Lerp(startPos, targetPos, linear) + new Vector3(0, height, 0);
            start.transform.rotation = Quaternion.Lerp(startRot, targetRot, linear);
    ...
    // Get Animation Frame
    public float GetCurrentAnimatorTime(Animator targetAnimator)
    {
        AnimatorStateInfo stateInfo = targetAnimator.GetCurrentAnimatorStateInfo(0);
        AnimatorClipInfo clipInfo = targetAnimator.GetCurrentAnimatorClipInfo(0)[0];

        float currentFrame = stateInfo.normalizedTime * (clipInfo.clip.length * clipInfo.clip.frameRate);

        return currentFrame;
    }

    // 모든 애니메이션 Bool 파라미터 세팅
    public void AllAnimatorParameterSetBool(Animator animator)
    {
    ...
    // SpriteObj Fade In or Out
    public IEnumerator FadeSpriteRenderer(GameObject spriteObj, bool isFadeIn)
    {
        ...
        SpriteRenderer fadeSpriteRenderer = spriteObj.GetComponent<SpriteRenderer>();
        fadeSpriteRenderer.color = new Color(1f, 1f, 1f, setAlpha);

        Color color = fadeSpriteRenderer.color;

        while (0f <= color.a && color.a <= 1f)
        {
            color.a += Time.deltaTime * 0.5f * flag;
    ...
    // SpriteObj Fade In or Out ~ fadeAction
    public IEnumerator FadeSpriteRenderers(GameObject spriteObj, bool isFadeIn, Action fadeAction)
    {
        spriteObj.SetActive(true);
    ...
    public IEnumerator SetSkybox(Material mt)
    {
        //isChange = true;
        ...
        Singleton.Instance.camera.GetComponent<Camera>().clearFlags = CameraClearFlags.Skybox;

        yield return new WaitForEndOfFrame();

        while (RenderSettings.skybox.GetFloat("_Exposure") <= 1)
        {
            RenderSettings.skybox.SetFloat("_Exposure", RenderSettings.skybox.GetFloat("_Exposure") + Time.deltaTime * 0.5f);
            yield return new WaitForEndOfFrame();
        }
    ...
    public IEnumerator SetSolidColor(Material mt)
    {
        // isChange = true;

        RenderSettings.skybox.SetFloat("_Exposure", 1f);

        while (RenderSettings.skybox.GetFloat("_Exposure") >= 0.1)
        {
            RenderSettings.skybox.SetFloat("_Exposure", RenderSettings.skybox.GetFloat("_Exposure") - Time.deltaTime * 0.5f);
            yield return new WaitForEndOfFrame();
        }
        ...
        RenderSettings.skybox = mt;
        Singleton.Instance.camera.GetComponent<Camera>().clearFlags = CameraClearFlags.SolidColor;
    ...
    {
        GameObject cube = GameObject.Find("Cube") == null ? GameObject.CreatePrimitive(PrimitiveType.Cube) : GameObject.Find("Cube");
        cube.transform.position = Singleton.Instance.camera.transform.position + new Vector3(0, 0, 10);

        cube.GetComponent<MeshRenderer>().sharedMaterial.color = condition ? rightColor : wrongColor;
    }

    // Text
    public void TextTest(string message, bool overwrite)
    {
        TextMeshPro tmp = GameObject.Find("Text") == null ? new GameObject("Text").AddComponent<TextMeshPro>() : GameObject.Find("Text").GetComponent<TextMeshPro>();
        tmp.GetComponent<RectTransform>().sizeDelta = new Vector2(300, 300);
        tmp.alignment = TextAlignmentOptions.Center;
        tmp.fontSize = 15;

        tmp.transform.parent = Singleton.Instance.camera.transform;
        tmp.transform.localPosition = new Vector3(0, 15, 30);
